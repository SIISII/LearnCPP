...
void  Init_Direct3D10_1()
{
    ...
    // Описание вершинного буфера. Он будет включать в себя те же три вершины,
    // но размещаться там, где выберет Direct3D в соответствии с задаваемыми
    // параметрами (в данном случае - в видеопамяти поближе к ГП и подальше от
    // ЦП, поскольку последний не будет работать с этим буфером вообще).
